...
def demo_custom_exceptions():
    ...
    try:
        field_errors = {"username": "用户名必须至少3个字符", "email": "邮箱格式无效"}
    ...
    try:
        raise BusinessLogicError(
            "无法删除有活跃订单的用户", details={"active_orders": 3, "user_id": "123"}
        )
...
def demo_flask_error_handling():
    ...
    def test_validation_error():
        raise ValidationError(
            "表单验证失败", field_errors={"email": "邮箱格式无效", "password": "密码太短"}
        )
    ...
    def test_business_logic():
        raise BusinessLogicError("无法执行此操作", details={"reason": "用户权限不足"})
