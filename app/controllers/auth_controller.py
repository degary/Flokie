...
def login():
    ...
    try:
        ...
        return (
            jsonify(
                {"error": "Login failed due to server error", "code": "INTERNAL_ERROR"}
            ),
            500,
        )
            ...
            jsonify(
                {
                    "success": True,
                    "message": result["message"],
                    "data": {"user": result.get("user")} if "user" in result else None,
                }
            ),
            200,
        )
...
def logout():
    ...
    try:
        ...
        return (
            jsonify(
                {"error": "Logout failed due to server error", "code": "INTERNAL_ERROR"}
            ),
            500,
        )
