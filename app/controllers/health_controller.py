...
def detailed_health_check():
    ...
    try:
        ...
        is_healthy = (
            ...
            and "error" not in system_resources
            and "error" not in app_info
        )
        ...
        if not is_healthy:
            ...
            if "error" in app_info:
                response["errors"].append(
                    {"component": "application_info", "message": app_info["error"]}
                )

        logger.debug(
            f"Detailed health check completed with status: {response['status']}"
        )
...
def database_health_check():
    ...
    try:
        ...
        logger.debug(
            f"Database health check completed with status: {response['status']}"
        )
    ...
    try:
        ...
        if "error" in system_resources:
            ...
            if system_resources["cpu"]["percent"] > cpu_threshold:
                is_healthy = False
                warnings.append(
                    f"High CPU usage: {system_resources['cpu']['percent']}%"
                )
            ...
            if system_resources["memory"]["percent"] > memory_threshold:
                is_healthy = False
                warnings.append(
                    f"High memory usage: {system_resources['memory']['percent']}%"
                )
            ...
            if system_resources["disk"]["percent"] > disk_threshold:
                is_healthy = False
                warnings.append(
                    f"High disk usage: {system_resources['disk']['percent']}%"
                )
...
def readiness_check():
    ...
    try:
        ...
        response = {
            "ready": is_ready,
            "timestamp": datetime.now(timezone.utc).isoformat(),
            "checks": {"database": db_status["status"] == "healthy"},
        }
...
def handle_not_found(error):
    """Handle 404 errors."""
    return (
        jsonify({"error": "Health check endpoint not found", "code": "NOT_FOUND"}),
        404,
    )
