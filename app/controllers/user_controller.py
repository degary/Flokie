...
class UserQuerySchema(Schema):
    ...
    sort_by = fields.Str(validate=validate.OneOf(['created_at', 'updated_at', 'username', 'email']), missing='created_at')
...
def get_users():
    ...
    try:
        ...
        logger.info(f"Retrieved {len(result['users'])} users (page {query_params['page']})")
...
def get_user(user_id):
    ...
    try:
        ...
        user = UserService.get_user_by_id(user_id, include_inactive=current_user.is_admin)
...
def delete_user(user_id):
    ...
    try:
        ...
        logger.info(f"User deleted (hard_delete={hard_delete}) by {current_user.username}")
...
def activate_user(user_id):
    ...
    try:
        ...
        user = UserService.activate_user(
            user_id=user_id,
            activated_by_user=current_user
        )
...
def deactivate_user(user_id):
    ...
    try:
        ...
        user = UserService.deactivate_user(
            user_id=user_id,
            deactivated_by_user=current_user
        )
...
def set_admin_status(user_id):
    ...
    try:
        ...
        logger.info(f"Admin privileges {status} for user: {user.username} by {current_user.username}")
...
def search_users():
    ...
    try:
        ...
        logger.info(f"Search completed: found {len(users_data)} users for term '{query_params['q']}'")
