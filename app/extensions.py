...
def init_extensions(app):
    ...
    # We'll handle documentation manually to avoid root path conflicts
    api.init_app(
        app,
        version="1.0",
        ...
        doc=False,  # Disable default doc route
        prefix="/api",
        add_specs=False,  # Disable default swagger.json route
        authorizations={
            "Bearer": {
                "type": "apiKey",
                "in": "header",
...
def configure_jwt_callbacks(app):
    ...
    def expired_token_callback(jwt_header, jwt_payload):
        logger.warning(
            f"[WARNING] Expired token access attempt from user: {jwt_payload.get('sub', 'unknown')}"
        )
        return {
            "error": "token_expired",
    ...
    def revoked_token_callback(jwt_header, jwt_payload):
        logger.warning(
            f"[WARNING] Revoked token access attempt from user: {jwt_payload.get('sub', 'unknown')}"
        )
        return {
            "error": "token_revoked",
    ...
    def token_not_fresh_callback(jwt_header, jwt_payload):
        logger.info(
            f"[INFO] Fresh token required for user: {jwt_payload.get('sub', 'unknown')}"
        )
        return {
    ...
    def user_lookup_callback(_jwt_header, jwt_data):
        ...
        try:
            ...
            if user and user.is_active and not user.is_account_locked():
                ...
                logger.warning(
                    f"User lookup failed for identity {identity}: user inactive or locked"
                )
                return None
