...
class BaseModel(db.Model, metaclass=BaseModelMeta):
    ...
    def __init__(self, **kwargs):
        ...
        for key, value in kwargs.items():
            if hasattr(self, key):
                ...
                logger.warning(
                    f"Attempted to set unknown attribute '{key}' on {self.__class__.__name__}"
                )
    ...
    @classmethod
    def from_dict(
        cls, data: Dict[str, Any], exclude_fields: Optional[List[str]] = None
    ):
        """
...
class ValidationMixin:
    ...
    def validate_unique_field(
        self, field_name: str, exclude_id: Optional[int] = None
    ) -> None:
        """
