...
class LoggingMiddleware:
    ...
    def _log_response(self, response, duration):
        ...
        if response.status_code >= 400:
            try:
                if response.content_length and response.content_length < 5000:  # 5KB limit
        ...
        if response.status_code >= 500:
            ...
            logger.warning(f"[WARNING] RESPONSE: {json.dumps(response_data, default=str)}")
        else:
    ...
    def _log_exception(self, exception):
        ...
        exception_data = {
            'timestamp': datetime.utcnow().isoformat(),
            'request_id': getattr(g, 'request_id', 'unknown') if has_request_context() else 'no-request',
        ...
        if isinstance(exception, HTTPException):
            ...
            logger.warning(f"[WARNING] HTTP_EXCEPTION: {json.dumps(exception_data, default=str)}")
        else:
            logger.error(f"[ERROR] EXCEPTION: {json.dumps(exception_data, default=str)}", exc_info=True)
...
class PerformanceMiddleware:
    ...
    def after_request(self, response):
        ...
        if duration > 1.0:
            ...
            logger.warning(f"[WARNING] Slow request detected: {request.path} took {duration:.2f}s")
        ...
        if duration > 5.0:
            logger.error(f"[ERROR] Very slow request: {request.method} {request.path} took {duration:.2f}s")
