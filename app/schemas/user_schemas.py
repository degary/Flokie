...
from .common_schemas import (
    BaseModelSchema, PaginationSchema, SortingSchema, SearchSchema,
    PasswordField, UsernameField, EmailField, NameField, TimestampMixin
)
...
class UserResponseSchema(BaseModelSchema, TimestampMixin):
    ...
    account_locked_until = fields.DateTime(allow_none=True, description="Account lock expiration")
...
class CreateUserRequestSchema(Schema):
    ...
    is_verified = fields.Bool(
        missing=False,
        description="Whether email should be pre-verified"
    )
...
class UpdateUserRequestSchema(Schema):
    ...
    is_admin = fields.Bool(
        allow_none=True,
        description="Whether user is admin (admin only)"
    )
...
class UserListResponseSchema(Schema):
    ...
    data = fields.Nested('UserListDataSchema', required=True, description="User list response data")
...
class UserListDataSchema(Schema):
    ...
    pagination = fields.Nested('PaginationInfoSchema', required=True, description="Pagination information")
...
class UserSingleResponseSchema(Schema):
    ...
    data = fields.Nested('UserDataSchema', required=True, description="User response data")


class UserDataSchema(Schema):
    """Schema for single user response data."""
    user = fields.Nested(UserResponseSchema, required=True, description="User information")
...
class UserStatisticsSchema(Schema):
    ...
    unverified_users = fields.Int(required=True, description="Number of unverified users")
    ...
    users_created_this_week = fields.Int(required=True, description="Users created this week")
    users_created_this_month = fields.Int(required=True, description="Users created this month")


class UserStatisticsResponseSchema(Schema):
    """Schema for user statistics response serialization."""
    success = fields.Bool(required=True, description="Operation success status")
    data = fields.Nested('UserStatisticsDataSchema', required=True, description="User statistics response data")


class UserStatisticsDataSchema(Schema):
    """Schema for user statistics response data."""
    statistics = fields.Nested(UserStatisticsSchema, required=True, description="User statistics")


class UserSearchResponseSchema(Schema):
    """Schema for user search response serialization."""
    success = fields.Bool(required=True, description="Operation success status")
    data = fields.Nested('UserSearchDataSchema', required=True, description="User search response data")
...
class UserDeleteResponseSchema(Schema):
    ...
    data = fields.Nested('UserDeleteDataSchema', required=True, description="User deletion response data")
