...
from .common_schemas import BaseModelSchema, PasswordField, UsernameField, EmailField, NameField
...
class LoginRequestSchema(Schema):
    ...
    remember_me = fields.Bool(
        missing=False,
        description="Whether to create longer-lived tokens"
    )
...
class RegisterRequestSchema(Schema):
    ...
    username = UsernameField(
        required=True,
        description="Desired username (3-80 characters)"
    )
    email = EmailField(
        required=True,
...
class LoginResponseSchema(Schema):
    ...
    data = fields.Nested('LoginDataSchema', required=True, description="Login response data")
...
class LoginDataSchema(Schema):
    ...
    tokens = fields.Nested(TokenResponseSchema, required=True, description="Authentication tokens")
...
class RegisterResponseSchema(Schema):
    ...
    data = fields.Nested('RegisterDataSchema', required=True, description="Registration response data")
...
class RefreshTokenResponseSchema(Schema):
    ...
    data = fields.Nested('RefreshTokenDataSchema', required=True, description="Token refresh response data")
...
class PasswordResetResponseSchema(Schema):
    ...
    data = fields.Nested('PasswordResetDataSchema', description="Password reset response data")
...
class EmailVerificationResponseSchema(Schema):
    ...
    success = fields.Bool(required=True, description="Email verification success status")
    message = fields.Str(required=True, description="Email verification result message")
    data = fields.Nested('EmailVerificationDataSchema', description="Email verification response data")
