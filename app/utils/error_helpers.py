...
from .exceptions import (
    ...
    ConflictError, BusinessLogicError, UserNotFoundError, InvalidCredentialsError,
...
def validate_field_length(data: Dict[str, Any], field_constraints: Dict[str, Dict[str, int]]) -> None:
    """
    ...
    for field, constraints in field_constraints.items():
        if field in data and data[field] is not None:
            ...
            if len(value) < min_length:
                ...
                field_errors[field] = f"Must be no more than {max_length} characters long"
...
def check_resource_exists(resource, resource_type: str, identifier: Optional[str] = None) -> None:
    """
...
def check_resource_ownership(user, resource, resource_type: str) -> None:
    ...
    if hasattr(resource, 'user_id') and resource.user_id != user.id:
        raise AuthorizationError(
            ...
            details={'resource_type': resource_type, 'resource_id': getattr(resource, 'id', None)}
        )


def handle_duplicate_resource(field: str, value: str, resource_type: str = "Resource") -> None:
    """
...
def validate_business_rule(condition: bool, message: str, details: Optional[Dict[str, Any]] = None) -> None:
    """
...
def validate_operation_allowed(operation: str, reason: str, details: Optional[Dict[str, Any]] = None) -> None:
    """
