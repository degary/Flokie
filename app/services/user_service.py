...
class UserService:
    ...
    def get_user_by_username(
        username: str, include_inactive: bool = False
    ) -> Optional[User]:
        """
    ...
    def get_users(
        ...
        """
        logger.debug(
            f"Getting users - page: {page}, per_page: {per_page}, search: {search}"
        )
        ...
        try:
            ...
            logger.debug(
                f"Retrieved {len(users_data)} users (page {page} of {pagination.pages})"
            )
            return result
            ...
            raise UserServiceError(
                "Failed to retrieve users", code="GET_USERS_ERROR", status_code=500
            )
    ...
    def create_user(
        ...
        if User.get_by_username(username):
            raise UserServiceError(
                "Username already exists", code="USERNAME_EXISTS", status_code=409
            )

        if User.get_by_email(email):
            raise UserServiceError(
                "Email address already exists", code="EMAIL_EXISTS", status_code=409
            )
        ...
        try:
            ...
            if created_by_user:
                logger.info(
                    f"User {user.username} created by {created_by_user.username}"
                )
            ...
            raise UserServiceError(
                "User creation failed", code="CREATE_USER_ERROR", status_code=500
            )

    @staticmethod
    def update_user(
        user_id: int, update_data: Dict[str, Any], updated_by_user: User = None
    ) -> User:
        """
        ...
        if not filtered_data:
            raise UserServiceError(
                "No valid fields to update", code="NO_UPDATE_FIELDS", status_code=400
            )
        ...
        try:
            ...
            if updated_by_user:
                logger.info(
                    f"User {user.username} updated by {updated_by_user.username}"
                )
            ...
            raise UserServiceError(
                "User update failed", code="UPDATE_USER_ERROR", status_code=500
            )

    @staticmethod
    def delete_user(
        user_id: int, deleted_by_user: User = None, soft_delete: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if deleted_by_user:
                logger.info(
                    f"User {user.username} deleted by {deleted_by_user.username}"
                )

            return {"message": message, "user_id": user_id, "soft_delete": soft_delete}

        except Exception as e:
            db.session.rollback()
            logger.error(f"Error deleting user {user_id}: {e}")
            raise UserServiceError(
                "User deletion failed", code="DELETE_USER_ERROR", status_code=500
            )
    ...
    def activate_user(user_id: int, activated_by_user: User = None) -> User:
        ...
        try:
            ...
            if activated_by_user:
                logger.info(
                    f"User {user.username} activated by {activated_by_user.username}"
                )
            ...
            raise UserServiceError(
                "User activation failed", code="ACTIVATE_USER_ERROR", status_code=500
            )
    ...
    def deactivate_user(user_id: int, deactivated_by_user: User = None) -> User:
        ...
        try:
            ...
            if deactivated_by_user:
                logger.info(
                    f"User {user.username} deactivated by {deactivated_by_user.username}"
                )
    ...
    def unlock_user_account(user_id: int, unlocked_by_user: User = None) -> User:
        ...
        try:
            ...
            if unlocked_by_user:
                logger.info(
                    f"User {user.username} unlocked by {unlocked_by_user.username}"
                )
            ...
            raise UserServiceError(
                "User unlock failed", code="UNLOCK_USER_ERROR", status_code=500
            )

    @staticmethod
    def set_user_admin_status(
        user_id: int, is_admin: bool, updated_by_user: User = None
    ) -> User:
        """
        ...
        if (
            updated_by_user
            and updated_by_user.id == user_id
            and updated_by_user.is_admin
            and not is_admin
        ):
            raise UserServiceError(
        ...
        try:
            ...
            logger.info(
                f"Admin privileges {status} for user: {user.username} (ID: {user.id})"
            )

            if updated_by_user:
                logger.info(
                    f"Admin status for {user.username} updated by {updated_by_user.username}"
                )
    ...
    @staticmethod
    def search_users(
        search_term: str, limit: int = 20, include_inactive: bool = False
    ) -> List[User]:
        """
    ...
    def check_user_permissions(user: User, required_permission: str) -> bool:
        ...
        """
        logger.debug(
            f"Checking permission '{required_permission}' for user: {user.username}"
        )
