...
class AuthService:
    ...
    def login(
        ...
        if not user.check_password(password):
            ...
            try:
                ...
                logger.error(
                    f"Failed to save login attempt for user {user.username}: {e}"
                )
                db.session.rollback()
    ...
    def refresh_token(current_user: User) -> Dict[str, Any]:
        ...
        if not current_user.is_active:
            logger.warning(
                f"Token refresh denied for inactive user: {current_user.username}"
            )
            raise AuthenticationError(
                "Account is deactivated", code="ACCOUNT_INACTIVE", status_code=403
            )

        if current_user.is_account_locked():
            logger.warning(
                f"Token refresh denied for locked user: {current_user.username}"
            )
            raise AuthenticationError(
                "Account is locked", code="ACCOUNT_LOCKED", status_code=423
            )
        ...
        try:
            ...
            logger.error(
                f"Failed to refresh token for user {current_user.username}: {e}"
            )
            raise AuthenticationError(
                "Token refresh failed", code="TOKEN_REFRESH_ERROR", status_code=500
            )
    ...
    def request_password_reset(email: str) -> Dict[str, Any]:
        ...
        if not email:
            raise AuthenticationError(
                "Email address is required", code="MISSING_EMAIL", status_code=400
            )
        ...
        try:
            ...
            logger.error(
                f"Failed to generate password reset token for email {email}: {e}"
            )
            db.session.rollback()
            raise AuthenticationError(
    ...
    def verify_email(token: str) -> Dict[str, Any]:
        ...
        if not token:
            raise AuthenticationError(
                "Verification token is required", code="MISSING_TOKEN", status_code=400
            )
    ...
    def change_password(
        ...
        if not user.check_password(current_password):
            logger.warning(
                f"Password change failed - invalid current password for user: {user.username}"
            )
            raise AuthenticationError(
        ...
        if remember_me:
            refresh_expires = timedelta(
                days=current_app.config.get(
                    "JWT_REFRESH_TOKEN_EXPIRES_DAYS_REMEMBER", 30
                )
            )
        else:
            refresh_expires = timedelta(
